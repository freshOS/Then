...
extension Promises {
    ...
    public static func zip<T:Sendable, U: Sendable>(_ p1: Promise<T>, _ p2: Promise<U>) -> Promise<(T, U)> {
        let p = Promise<(T, U)>()
        Task(priority: .background) {
            async let p1Result:T = awaitPromise(p1)
            async let p2Result:U = awaitPromise(p2)
            do {
                let results = try await (p1Result, p2Result)
                p.fulfill(results)
            } catch {
                p.reject(error)
            }
